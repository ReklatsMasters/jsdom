...
class DocumentImpl extends NodeImpl {
  ...
  get characterSet() {
    ...
    for (const meta of descendants(this.head, "META", false)) {
      if (meta.getAttribute && meta.getAttribute("charset")) {
        this._characterSet = meta.getAttribute("charset");
        return this._characterSet;
      }
      if (meta.getAttribute && meta.getAttribute("http-equiv")) {
        if (meta.getAttribute("http-equiv").toLowerCase() === "content-type") {
          if (meta.getAttribute("content")) {
            const content = meta.getAttribute("content");
            const matches = content.match(/charset=([\w\d-]+)/i);

            if (matches.length >= 2) {
              this._characterSet = matches[1];
              return this._characterSet;
            }
          }
        }
